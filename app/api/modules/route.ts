...
// ---------------- DELETE MODULE (teacher/admin only) ----------------
export async function DELETE(req: Request) {
  try {
    ...
    const user = await User.findById(decoded.userId)
    if (!user || (user.role !== 'teacher' && user.role !== 'admin')) {
      return NextResponse.json({ error: 'Forbidden: Only teachers/admins can delete modules' }, { status: 403 })
    }

    // Get module ID from query parameters
